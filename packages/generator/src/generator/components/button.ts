...
export function init(
  ...
  buttonVariables['btnBg'] = button.createVariable('btnBg', 'transparent');
  ...
  button.styles({
    ...
    border: args('solid', '1.5px', buttonVariables['btnBorderColor']),
  ...
  if (enabled.outlineVariant) {
    const buttonOutlineVariants = config.baseColors.flatMap((v) => {
      let outlineButton = className(`btn-outline-${v}`)
        ...
        .setVariable(buttonVariables['btnBg'], 'transparent')
        .setVariable(
          buttonVariables['btnBorderColor'],
          value(globalVariables[`${v}Color`]),
        );
